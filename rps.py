...
capitalize() in rps else evl_inp_cho(input(pro)))('-e')).lower() else ([f"Win: {usr.capitalize()} vs {cho}", scr.
append(1)][0] if ({k: ['s', 'k', 'r'][i] for i, k in enumerate([i[-1] for i in rps])})[usr[-1]] == cho[-1] else [f"Lose\
